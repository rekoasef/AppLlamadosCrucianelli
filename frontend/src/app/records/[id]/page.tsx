...
interface DetailedCallRecord {
  ...
  callerType: { id: string; name: string; } | null;
  machineType: { id: string; name: string; } | null;
  dealership: { id: string; name: string; } | null;
  inquiryArea: { id: string; name: string; } | null;
  responseReason: { id: string; name: string; } | null;
  contactChannel: { id: string; name: string; } | null;
  durationRange: { id: string; name: string; } | null;
  urgencyLevel: { id: string; name: string; } | null;
...
export default function RecordDetailPage() {
  ...
  if (error && !record) return <div className="p-8 text-center text-red-500">Error: {error}</div>;
  ...
  return (
    <div className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-bold text-crucianelli-dark">
            Detalle del Registro ({record.businessUnit.name})
          </h1>
        ...
        <div className="flex items-center gap-4">
          <Link
            ...
            className="px-4 py-2 rounded-md text-sm font-medium text-white bg-crucianelli-red hover:bg-opacity-90"
          >
            Editar
          </Link>
          <Link
            href="/records"
            className="text-sm font-medium text-crucianelli-red hover:text-opacity-80"
          >
            &larr; Volver
          </Link>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="md:col-span-2 space-y-6">
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            <h2 className="text-lg font-semibold text-crucianelli-dark border-b pb-3 mb-4">Información del Producto</h2>
            <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
              ...
              <DetailItem label="Tipo de Producto LEAF" value={record.specificData?.leafProductTypeId} />
              <DetailItem label="Número de Serie LEAF" value={record.specificData?.leafSerialNumber} />
            </dl>
          </div>
          <div className="bg-white p-6 rounded-lg shadow">
            <h2 className="text-lg font-semibold text-crucianelli-dark border-b pb-3 mb-4">Detalle de la Consulta</h2>
            <p className="text-sm text-crucianelli-dark prose max-w-none">{record.observations || 'Sin observaciones.'}</p>
          </div>
        </div>
        ...
        <div className="md:col-span-1 space-y-6">
          <div className="bg-white p-6 rounded-lg shadow">
            ...
            <dl className="space-y-4">
              <DetailItem label="Estado Actual" value={statusTranslations[record.status]} />
              <DetailItem label="Área de Consulta / Sistema" value={record.inquiryArea?.name} />
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            <h2 className="text-lg font-semibold text-crucianelli-dark border-b pb-3 mb-4">Cambiar Estado</h2>
            <div className="space-y-4">
              <label htmlFor="status" className="block text-sm font-medium text-crucianelli-gray">Nuevo Estado</label>
              <select
                id="status"
                value={newStatus}
                onChange={(e) => setNewStatus(e.target.value)}
                className="mt-1 block w-full border-gray-300 rounded-md shadow-sm text-crucianelli-dark"
              >
                <option value="OPEN">Abierto</option>
                <option value="IN_PROGRESS">En Progreso</option>
                <option value="CLOSED">Cerrado</option>
                <option value="PENDING_CLIENT">Pendiente del Cliente</option>
              </select>
              <button
                onClick={handleStatusChange}
                disabled={isUpdatingStatus || newStatus === record.status}
                className="w-full px-4 py-2 text-sm font-medium text-white bg-crucianelli-red rounded-md hover:bg-opacity-90 disabled:bg-opacity-50"
              >
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            ...
            <dl className="space-y-4">
              <DetailItem label="Creado por" value={record.createdByUser.name} />
              ...
              <DetailItem label="Atendido por" value={record.handledBy?.name} />
            </dl>
          </div>
        </div>
      </div>
